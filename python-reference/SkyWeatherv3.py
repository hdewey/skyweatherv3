...
import time
aio = Client('troylanier','5e0621153eff3da0c87717d1fc2fdd8f901a3568')
	...
	if (config.BME680_Present):	
		try:
			...
			bmp180SeaLevel = BME680_Functions.getSeaLevelPressure(config.BMP280_Altitude_Meters, bmp180Pressure)
                        ...
                        aio.send('weather-station.temperature', bmp180Temperature)
                        aio.send('weather-station.humidity', bmp180Humidity)
                        aio.send('weather-station.pressure', bmp180Pressure)
                        
                        
                        
                        print "sent adafruit data ************************"
		except:
        		print("Unexpected error:", sys.exc_info()[0])
